...
void emscripten_async_load_script(const char *script, em_callback_func onload,
...
void emscripten_set_main_loop_arg(em_arg_callback_func func, void *arg, int fps,
                                  int simulate_infinite_loop);
...
typedef void (*em_socket_error_callback)(int fd, int err, const char *msg,
                                         void *userData);

void emscripten_set_socket_error_callback(void *userData,
                                          em_socket_error_callback callback);
void emscripten_set_socket_open_callback(void *userData,
                                         em_socket_callback callback);
void emscripten_set_socket_listen_callback(void *userData,
                                           em_socket_callback callback);
void emscripten_set_socket_connection_callback(void *userData,
                                               em_socket_callback callback);
void emscripten_set_socket_message_callback(void *userData,
                                            em_socket_callback callback);
void emscripten_set_socket_close_callback(void *userData,
                                          em_socket_callback callback);

void _emscripten_push_main_loop_blocker(em_arg_callback_func func, void *arg,
                                        const char *name);
void _emscripten_push_uncounted_main_loop_blocker(em_arg_callback_func func,
...
// __attribute__((deprecated("This variant does not allow specifying the
// target canvas", "Use emscripten_set_canvas_element_size() instead"))); void
// emscripten_get_canvas_size(int *width, int *height, int *isFullscreen)
// __attribute__((deprecated("This variant does not allow specifying the
// target canvas", "Use emscripten_get_canvas_element_size() and
...
void emscripten_idb_async_load(const char *db_name, const char *file_id,
                               void *arg, em_idb_onload_func onload,
                               em_arg_callback_func onerror);
void emscripten_idb_async_store(const char *db_name, const char *file_id,
                                void *ptr, int num, void *arg,
                                em_arg_callback_func onstore,
                                em_arg_callback_func onerror);
void emscripten_idb_async_delete(const char *db_name, const char *file_id,
                                 void *arg, em_arg_callback_func ondelete,
                                 em_arg_callback_func onerror);
typedef void (*em_idb_exists_func)(void *, int);
void emscripten_idb_async_exists(const char *db_name, const char *file_id,
                                 void *arg, em_idb_exists_func oncheck,
                                 em_arg_callback_func onerror);

// IDB "sync"

void emscripten_idb_load(const char *db_name, const char *file_id,
                         void **pbuffer, int *pnum, int *perror);
void emscripten_idb_store(const char *db_name, const char *file_id,
                          void *buffer, int num, int *perror);
void emscripten_idb_delete(const char *db_name, const char *file_id,
                           int *perror);
void emscripten_idb_exists(const char *db_name, const char *file_id,
                           int *pexists, int *perror);

void emscripten_idb_load_blob(const char *db_name, const char *file_id,
                              int *pblob, int *perror);
void emscripten_idb_store_blob(const char *db_name, const char *file_id,
                               void *buffer, int num, int *perror);
...
int emscripten_run_preload_plugins(const char *file,
...
void emscripten_run_preload_plugins_data(
    char *data, int size, const char *suffix, void *arg,
...
#define emscripten_async_prepare(...)                                          \
  _Pragma("GCC error(\"emscripten_async_prepare has been replaced by "         \
          "emscripten_run_preload_plugins\")")
#define emscripten_async_prepare_data(...)                                     \
  _Pragma("GCC error(\"emscripten_async_prepare_data has been replaced by "    \
...
void emscripten_call_worker(worker_handle worker, const char *funcname,
                            char *data, int size,
                            em_worker_callback_func callback, void *arg);
...
// Forward declare FILE from musl libc headers to avoid needing to #include
// <stdio.h> from emscripten.h
struct _IO_FILE;
typedef struct _IO_FILE FILE2;

char *emscripten_get_preloaded_image_data(const char *path, int *w, int *h);
char *emscripten_get_preloaded_image_data_from_FILE(FILE2 *file, int *w,
                                                    int *h);
...
// Asynchronous version of dlopen.  Since WebAssembly module loading in
// general is asynchronous the normal dlopen function can't be used in all
// situations.
